...
fs.readFile('client_secret.json', function processClientSecrets(err, content) {
  ...
  //authorize(JSON.parse(content), cleanRows);
});

exports.do = function( clientSecretJson, fxn ) {
    fs.readFile(clientSecretJson, function processClientSecrets(err, content) {
      if (err) {
        console.log('Error loading client secret file: ' + err);
        return;
      }
      // Authorize a client with the loaded credentials, then call the
      // Google Sheets API.
      exports.authorize(JSON.parse(content), fxn);
    });
}
