...
var RECENT_SEARCHES_INTERVAL = 10000;
var RECENT_SEARCHES_MAX_SIZE = 15;
var NOTIFICATION_TIMEOUT = 6000;
var NOTIFICATION_SLIDEUP_SPEED = 1500;
...
Queue.prototype = {
    add: function(obj) {
        if (this.objs.length >= this.max_size) {
            this.remove();
        };
...
(function() {
    $(document).ready(function() {
        var container = $('.activity-feed-container');
        if (container.length) {
            var queue = new Queue(RECENT_SEARCHES_MAX_SIZE);

            function update_feed() {
                get_recent_searches(function(results) {
                    if (!results) {
                        queue.empty();
                        return false;
                    };
            ...
            function update_container() {
                container.empty();
                if (queue.size() === 0) {
                    container.html('No activity to show');
                };
                var html = '<ul class="list-unstyled activity-feed">';
                var objs = queue.all();
                for (var i = 0; i < objs.length; i++) {
                    html += '<li>' + '<a href="' + objs[i]['request_uri'] + '">' + objs[i]['query'] + '</a>' + '</li>'
                };
                html += '</ul>';
                container.html(html);
            };

            function get_recent_searches(callback) {
                $.ajax({
                    url: '/api/recent_searches',
                    method: 'GET',
                    dataType: 'json',
...
(function() {
    $(document).ready(function() {
        ...
        if (notification.length) {
            setTimeout(function() {
                notification.fadeOut(NOTIFICATION_SLIDEUP_SPEED, function() {
                    notification.remove();
                });
...
(function() {
    $(document).ready(function() {
        var forms = $('.pflag-form');
        if (forms.length) {
            function submit_pflag(pflag) {};
            $(forms).each(function() {
                var form = $(this);
                var phone = form.find('.pflag-phone').text();
                var items = form.find('.pflag-label');
                items.each(function() {
                    $(this).click(function() {
                        var pflag = $(this);
                        if (!pflag.hasClass('pflag-disabled')) {
                            pflag_txt = pflag.text().trim();
                            items.each(function() {
                                $(this).unbind('click');
                                if (!$(this).is(pflag)) {
                                    $(this).addClass('pflag-disabled').fadeTo(1500, 0.2);
                                }
                            });
                            if (phone.length) {
                                $.ajax({
...
(function() {
    var container = $('.modal');
    if (container.length) {
        container.prop('class', 'modal fade').addClass($(this).data('direction'));
